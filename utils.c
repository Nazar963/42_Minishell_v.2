...
/*   Updated: 2023/10/11 17:23:34 by naal-jen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (sh->doexit != -1 || shft_redirections(&cmds[loco()->counter], sh, &doset))
	{
		pipe(pp);
		close(*(pp + 1));
...
char	**execution_and_bonus_helper_1(int doset, char **cmds, t_shell_stuff *sh, int *pp)
{
	...
	while (++counter < 2)
	{
		...
		if (counter == 2 && sh->lststatus == 1
		|| sh->lststatus == 127 || sh->lststatus == 126 && loco()->exit != 0)
			break ;
